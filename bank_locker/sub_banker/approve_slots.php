...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $action = $_POST['action'];
    $subbanker_name = 'Sub Banker: ' . $_SESSION['subbanker_name'];
    
    if ($action === 'approve') {
        $stmt = $conn->prepare("UPDATE access_log SET status='approved', approved_by=? WHERE id=?");
        $stmt->bind_param("si", $subbanker_name, $access_id);
        if ($stmt->execute()) {
            $msg = "Slot booking approved successfully!";
            logActivity($conn, 'sub_banker', $_SESSION['subbanker_id'], $_SESSION['subbanker_name'], "Approved access slot booking #$access_id");
        } else {
        ...
        if ($stmt->execute()) {
            ...
            logActivity($conn, 'sub_banker', $_SESSION['subbanker_id'], $_SESSION['subbanker_name'], "Rejected access slot booking #$access_id");
        } else {
            $err = "Error: " . $conn->error;
        }
    }
...
.slot-available {
  border-color: #0d9488; /* Teal border for Sub Banker panel theme */
...
<div class="card mb-20">
  ...
  <div class="card-body">
    <form method="GET" class="check-date-form">
      ...
      <?php endif; ?>
      <label style="font-weight:600;color:var(--teal-dark);white-space:nowrap;">Select Date:</label>
      ...
      <button type="submit" class="btn btn-teal" style="white-space:nowrap;">Check Slots</button>
    </form>
    ...
    <div class="slots-legend">
      <span><span class="legend-dot" style="background:#0d9488;"></span>Available &mdash; slot is free for booking</span>
