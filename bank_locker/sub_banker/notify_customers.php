...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action']) && $_POST['action'] === 'send') {
        ...
        $sender_type = 'sub_banker';
        $sender_id = $_SESSION['subbanker_id'];
        $sender_name = $_SESSION['subbanker_name'];
        ...
        if ($stmt->execute()) {
            ...
            logActivity($conn, 'sub_banker', $_SESSION['subbanker_id'], $_SESSION['subbanker_name'], "Sent notification (#$notif_id) to $target", "Channels: $channels_str");
        } else {
        ...
        // Verify notification was sent by this sub-banker
        $stmt = $conn->prepare("DELETE FROM notifications WHERE id=? AND sender_type='sub_banker' AND sender_id=?");
        $stmt->bind_param("ii", $notif_id, $_SESSION['subbanker_id']);
        if ($stmt->execute()) {
            $msg = "Notification deleted successfully.";
            logActivity($conn, 'sub_banker', $_SESSION['subbanker_id'], $_SESSION['subbanker_name'], "Deleted notification #$notif_id");
        } else {
    ...
    <form method="POST">
      ...
      <div class="form-grid">
        ...
        <div class="form-group">
          ...
          <input type="text" name="title" class="form-control" required placeholder="e.g. System Maintenance Update">
        </div>
      </div>
      ...
      <button type="submit" class="btn btn-teal">📢 Dispatch Notification</button>
    </form>
  </div>
</div>
          ...
          <?php while ($h = $history->fetch_assoc()): ?>
            <tr>
              ...
              <td>
                ...
                <small style="color:#888;"><?= ucfirst(str_replace('_', ' ', $h['sender_type'])) ?></small>
              </td>
              <td>
              ...
              <td>
                <div style="font-weight: 700; color: var(--teal-dark); margin-bottom: 4px;"><?= htmlspecialchars($h['title']) ?></div>
              ...
              <td>
                <?php if ($h['sender_type'] === 'sub_banker' && $h['sender_id'] == $_SESSION['subbanker_id']): ?>
                ...
                <?php else: ?>
                  <span style="color:#aaa; font-size:11px; font-style:italic;">Unauthorized</span>
                <?php endif; ?>
              </td>
            </tr>
          <?php endwhile; ?>
...
<?php
$conn->close();
require_once '../includes/footer_subbanker.php';
?>
