...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($chk['status'] !== 'available') {
        ...
        try {
            ...
            $conn->commit();
            logActivity($conn, 'sub_banker', $_SESSION['subbanker_id'], $_SESSION['subbanker_name'], 'Allocated Locker', "Allocated locker (ID: $lock_id) to Customer (ID: $cust_id) - Alloc No: $alloc_no");
            $msg = "Locker allocated! Allocation No: $alloc_no";
        } catch (Exception $e) {
            $conn->rollback();
